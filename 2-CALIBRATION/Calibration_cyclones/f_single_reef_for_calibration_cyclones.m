...
function [META, REEF, RESULT, RECORD] = f_single_reef_for_calibration_cyclones(RESTORATION, warming_scenario, ...
...
%% Initial conditions for cyclone calibration
load('AIMS_LTMP_transects_calibration_cyclones.mat');
select = find(AIMSLTMPtransectscalibrationcyclones.STORM_DATE == 'BEFORE');
init_coral_cover = table2array(AIMSLTMPtransectscalibrationcyclones(select,7:12))/100;
varSD = std(init_coral_cover,[],1)/sqrt(size(init_coral_cover,1));

CYCLONE_CAT = AIMSLTMPtransectscalibrationcyclones.CATEGORY(select);
META.nb_reefs = size(init_coral_cover,1);
META.reef_ID = 1:META.nb_reefs;
